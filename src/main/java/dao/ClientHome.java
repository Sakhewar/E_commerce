...
public class ClientHome {
	...
	public Client findById(entities.ClientId id) {
		log.debug("getting Client instance with id: " + id);
		try {
			Client instance = (Client) sessionFactory.getCurrentSession().get("dao.Client", id);
			if (instance == null) {
	...
	public List findByExample(Client instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("dao.Client").add(Example.create(instance))
					.list();
